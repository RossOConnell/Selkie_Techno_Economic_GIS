...
function calcCapex1() {
    ...
    if (
        ...
        var installationCosts1 =
            ...
            ((parseFloat(defaultCableInterLength.value) * parseFloat(defaultCableInstall.value)) + (parseFloat(defaultCableIntraLength.value) * (parseFloat(defaultArrayCableCost.value))) + (parseFloat(defaultOnElec.value) * parseFloat(defaultFarmRating.value)) + (parseFloat(defaultOffLog.value) * parseFloat(defaultFarmRating.value)))
...
function calc_te1() {
    ...
    for (let yr = 1; yr < n; yr++) {
        ...
        opexDiscounted = (parseFloat(opex)) / (1 * (1 + (parseFloat(defaultDiscountRate.value) / 100)) ** yr);
    ...
    let df = 1 / 1 * (1 + (parseFloat(defaultDiscountRate.value) / 100)) ** n;
    console.log("df: " + df);
    ...
    benefits = (parseFloat(E)) * (parseFloat(price) * 1000);
    console.log("benefits " + benefits);


    // Calculate LCOE and NPV 

    LCOElong1 = parseFloat(LCS_oef) / parseFloat(E);
...
function calc_te3() {
    ...
    let totalcapex = parseFloat(totcapex.value);
    let n = parseFloat(defaultProjectLife.value);
    let price = parseFloat(defaultElecPrice.value) / 100;
    let E = parseFloat(totaep.value) * parseFloat(defaultProjectLife.value);
    let opex = parseFloat(totopex.value);
    ...
    let LCS_oef = parseFloat(totalcapex) + parseFloat(sumOpexDiscounted) + parseFloat(decexDiscount);
    console.log("LCS_oef: " + LCS_oef);
...
require([
    ...
    var availabilityGroupLayer = new GroupLayer({
        title: "Availability",
        visible: true,
        visibilityMode: "inclusive",
    ...
    var accessibilityGroupLayer = new GroupLayer({
        title: "Accessibility",
        visible: true,
        visibilityMode: "inclusive",
    ...
    function buildPopupContent1(feature) {
        ...
        div.innerHTML =
            ...
            '<tr><td><i title = "Length of mooring for each WEC.">Mooring Length: </i><td><input type="number" title = "Default value reference: N/A." name="defaultMoorLength" id="defaultMoorLength" style="width: 6em" value="0" min=1 max=50000000000 onchange="calcAep1(); calcCapex1(); calcOpex1()"/><span id="defaultMoorLength_errors"><i> m/WEC </i></span></td></tr>' +
    ...
    function buildPopupContent2(feature) {
        ...
        div.innerHTML =
            ...
            '<tr><td><i title = "Length of mooring for each TEC.">Mooring Length: </i><td><input type="number" title = "Default value reference: N/A." name="defaultMoorLength" id="defaultMoorLength" style="width: 6em" value="0" min=1 max=50000000000 onchange="calcAep2(); calcCapex2(); calcOpex2()"/><span id="defaultMoorLength_errors"><i> m/TEC </i></span></td></tr>' +
    ...
    function buildPopupContent3(feature) {
        ...
        div.innerHTML =
            ...
            '<tr><td><i title = "The number of Tidal Energy Converters to be deployed.">Number of TECs: </i><td><input type="number" title = "Default reference value: N/A." name="defaultTec" id="defaultTec" style="width: 5em" value="' + Math.round(105 / (attributes.Rating / 1000)) + '" min=1 max=1000 onchange="exportOptionFarmRating3(); calcAep3(); calcCapex3(); calcOpex3()"/><span id="defaultTec_errors"><i> units </i></span></td></tr>' +
            ...
            '<tr><td><i title = "Length of mooring for each TEC.">Mooring Length: </i><td><input type="number" title = "Default value reference: N/A." name="defaultMoorLength" id="defaultMoorLength" style="width: 6em" value="0" min=1 max=50000000000 onchange="calcAep3(); calcCapex3(); calcOpex3()"/><span id="defaultMoorLength_errors"><i> m/TEC </i></span></td></tr>' +
    ...
    view.when(function () {
        ...
        layerList.on("trigger-action", function (event) {
            ...
            } else if ((id === "information") && (event.item.layer.title === "Annual Accessibility (%): 1.5m Hs and 20 m/s wind")) {
                // if the information action is triggered, then
                // open the item details page of the service layer
                window.open("https://marei.maps.arcgis.com/home/item.html?id=3aacdfe76cc948178916476d387c3b8a");
            } else if ((id === "increase-opacity") && (event.item.layer.title === "Annual Accessibility (%): 1.5m Hs and 20 m/s wind")) {
                // if the increase-opacity action is triggered, then
                // increase the opacity of the GroupLayer by 0.25
            ...
            } else if ((id === "information") && (event.item.layer.title === "Annual Availability (%): 1.5m Hs and 20 m/s wind")) {
                // if the information action is triggered, then
                // open the item details page of the service layer
                window.open("https://marei.maps.arcgis.com/home/item.html?id=e3c62b6c2c47431eb05d1c9c2bf7cb0b");
            } else if ((id === "increase-opacity") && (event.item.layer.title === "Annual Availability (%): 1.5m Hs and 20 m/s wind")) {
                // if the increase-opacity action is triggered, then
                // increase the opacity of the GroupLayer by 0.25
